...
async def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    app_logger.info(f"🔐 新用户注册成功 | username={user_data.username} | id={user.id}")
    return user
...
async def login(
    ...
    if not user:
        app_logger.warning(f"🔐 登录失败 | username={form_data.username} | 用户名或密码错误")
        raise HTTPException(
    ...
    app_logger.info(f"🔐 登录成功 | username={user.username} | id={user.id}")
...
async def login_json(user_data: UserLogin, db: Session = Depends(get_db)):
    ...
    if not user:
        app_logger.warning(f"🔐 登录失败(JSON) | username={user_data.username} | 用户名或密码错误")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="用户名或密码错误",
        )
    ...
    app_logger.info(f"🔐 登录成功(JSON) | username={user.username} | id={user.id}")
    return {"access_token": access_token, "token_type": "bearer"}
