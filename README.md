...
- ✅ **自动化准确率评估工具**：新增 `scripts/analyze_citation_accuracy.py` 脚本，自动化评估 AI 回答中法律引用的准确性
- ✅ **多格式引用识别**：支持识别 8+ 种法律引用格式（《法律名》第XX条、法律名第XX条、括号格式等）
- ✅ **智能法律名称匹配**：内置 20+ 法律文件的别名字典，自动处理简称和全称的映射（如"道交法"↔"道路交通安全法"）
- ✅ **数字到中文转换**：支持阿拉伯数字条号与中文汉字的互相转换验证（如 232 ↔ 二百三十二）
- ✅ **三格式报告输出**：自动生成 JSON（机器可读）、TXT（人类友好）、CSV（数据分析）三种格式报告
- ✅ **系统性能基线**：现有 110 条对话的评估显示 **89.6% 的引用准确率**（1296 条引用，1193 条已验证）
- 📊 **详细使用指南**：完整文档见 [docs/CITATION_ACCURACY_GUIDE.md](docs/CITATION_ACCURACY_GUIDE.md)
...
# 仅分析最近10条对话（测试用）
python scripts/analyze_citation_accuracy.py --limit 10
```
...
- ✅ **重命名对话**：侧边栏历史对话下拉菜单新增「重命名」选项，点击后弹出输入框，修改后实时同步更新对话标题

##### 2. 历史对话显示优化

- ✅ **显示条数增加**：侧边栏历史对话列表最大高度从 300px 提升至 480px，约可同时展示 11-12 条对话，无需频繁滚动

##### 3. 对话搜索

- ✅ **快速搜索对话**：历史对话列表上方新增搜索框，输入关键词实时过滤匹配对话，快速找到目标对话

##### 4. 用户密码修改

- ✅ **密码修改**：侧边栏用户区域新增设置按钮，点击弹出密码修改对话框（当前密码 / 新密码 / 确认密码），修改成功后自动跳转重新登录

##### 5. 文档问答优化

- ✅ **跳过法律相关性检查**：上传文档时不再进行法律相关性判断，直接进入 RAG 流程
- ✅ **智能检索重写**：上传文档后，由大模型将用户问题+文档内容重写为精准的法律检索 query，确保向量库命中相关法条

##### 7. 日志系统

- ✅ **文件日志**：新增 `logs/app.log` 文件日志，按天滚动保留 7 天
- ✅ **HTTP 请求日志**：全局中间件记录每个 API 请求的路径、状态码、响应时间
- ✅ **认证日志**：记录注册/登录/登出/密码修改等事件
- ✅ **控制台完整输出**：显示完整的法律上下文、网页上下文、多查询重写子问题、以及大模型实际收到的完整 Prompt
...
- **消息评价**：对 AI 回答进行 👍👎 反馈评价
- **收藏管理**：保存重要问答，支持分页浏览
- **对话管理**：创建、删除、重命名、搜索、导出对话，支持分页加载
- **密码修改**：用户自助修改密码，修改后自动重新登录
- **实时流式**：逐字显示 AI 回答
...
### 2026-03-02 - 对话管理优化 & 系统增强

#### 对话体验

- ✅ 对话重命名功能（侧边栏下拉菜单 → 弹窗输入 → 实时同步）
- ✅ 历史对话列表高度扩展（300px → 480px，约 11-12 条无需滚动）
- ✅ 对话搜索（侧边栏搜索框，实时关键词过滤）

#### 用户功能

- ✅ 密码修改（设置按钮 → 当前密码/新密码/确认 → 修改后自动重新登录）

#### Prompt 与检索优化

- ✅ 四段式结构化回答（法律依据 → 案情分析 → 应对建议 → 核心结论）
- ✅ 禁止 HTML 标签，强制 Markdown 格式输出
- ✅ 文档问答跳过法律相关性检查
- ✅ 文档检索智能重写（大模型将用户问题+文档内容重写为精准法律 query）

#### 日志系统

- ✅ `logs/app.log` 文件日志（按天滚动，保留 7 天）
- ✅ HTTP 请求中间件日志（路径、状态码、响应时间）
- ✅ 认证事件日志（注册/登录/登出/密码修改）
- ✅ 控制台完整输出（法律上下文 + 网页上下文 + 多查询重写子问题 + 完整 Prompt）
