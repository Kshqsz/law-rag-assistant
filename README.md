...
- ✅ **差评原因选项**：用户在点击 👎「踩」时，可多选预设问题类型（如准确率较差、法律依据不充分、回答不完整、表达语气不佳、与问题不相关）
- ✅ **自定义问题描述**：支持用户在点踩时补充自由文本说明，描述回答具体问题
- ✅ **反馈内容入库**：前端已将点踩原因与补充说明拼接后写入反馈接口 `comment` 字段，后端/数据库 `feedbacks.comment` 完整保存
- ✅ **交互保持一致**：点赞仍保持一键提交；再次点击相同反馈可取消评价
...
- **消息评价**：对 AI 回答进行 👍👎 反馈评价
- **差评细化反馈**：点踩时支持原因多选 + 自定义文字说明，反馈写入数据库 comment 字段
- **收藏管理**：保存重要问答，支持分页浏览
